...
NSString* const LSDownloadNetworkSpeedNotificationKey = @"LSDownloadNetworkSpeedNotificationKey";
NSString* const LSUploadNetworkSpeedNotificationKey = @"LSUploadNetworkSpeedNotificationKey";

@interface LSNetworkSpeed () {
    //总网速
    uint32_t _iBytes;
    uint32_t _oBytes;
    uint32_t _allFlow;
...
@implementation LSNetworkSpeed

static LSNetworkSpeed* instance = nil;
...
- (void)checkNetworkSpeed
{
    ...
    if (_iBytes != 0) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:LSDownloadNetworkSpeedNotificationKey object:nil];
    }
    _iBytes = iBytes;

    if (_oBytes != 0) {
        _uploadNetworkSpeed = [[self stringWithbytes:oBytes - _oBytes] stringByAppendingString:@"/s"];
        [[NSNotificationCenter defaultCenter] postNotificationName:LSUploadNetworkSpeedNotificationKey object:nil];
    }
    _oBytes = oBytes;
}
@end
