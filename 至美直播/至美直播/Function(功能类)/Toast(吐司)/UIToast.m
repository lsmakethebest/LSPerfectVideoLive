...
- (id)initWithMessage:(NSString *)message {
  if (self) {
    ...
    CGFloat msg_x = (SCREEN_W - msg_size.width - 2 * LSToastBorder) / 2;
    CGFloat msg_y = SCREEN_H * 3 / 4;
  ...
  if (IOS_VERSION < 7.0) {
    stringSize =
        [string sizeWithFont:LSToastContentFont
            constrainedToSize:CGSizeMake(SCREEN_W - 2 * LSToastBorder, 200)
                lineBreakMode:NSLineBreakByWordWrapping];
  } else {
    stringSize =
        [string
            boundingRectWithSize:CGSizeMake(SCREEN_W - 2 * LSToastBorder, 200)
